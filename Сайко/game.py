...
class Ball(Figure):
    ...
    def draw(self):
        canv.create_oval(self.x-self.r, self.y-self.r, self.x +
                         self.r, self.y+self.r, fill=self.color, width=0)
        self.yv += g
        self.x += self.xv
        self.y += self.yv
...
def move():
    ...
    canv.create_text(int(int(xpole)/2), 100,
                     text=f"Ваши очки: {ochki}",  # Отрисовываем очки
